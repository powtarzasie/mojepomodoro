...
> i powody. | Stan: **v1.3.3 (naprawy audytu A1–A7), 2026-07-15** — v1.3.2 WYDANA (onboarding); `package.json` = 1.3.3 (bump w audycie finalnym) | Źródło prawdy: `C:\PROJEKTY\aplikacje\mojepomodoro`
...
| Funkcja | Gdzie | Uwagi / decyzje | Zweryfikowano |
|---|---|---|---|
| Model zadania | `{id, name, totalMinutes}` (`sanitizeTask`) | budżet minut na zadanie; `taskSpent` limit 480 min — **egzekwowany także w `tick()`** (A3 v1.3.3: `S.taskSpent = min(...+przyrost, 480*60)`, wcześniej clamp był tylko przy wczytaniu); nazwa cięta do 200 znaków (main; bufor managera pokazuje dłuższą do odświeżenia — kosmetyka, odnotowane w audycie 07.2026) | 2026-07-15 (harness A3) |
...
| Sanityzacja przy wczytaniu | `sanitizeTask`/`sanitizeSettings`/`clampInt`/`dedupeIds` | currentIdx może == length („wszystko ukończone"). Testy 07.2026: całkiem zepsuty JSON → defaulty + bramka terms; dzikie wartości → wszystkie clampy działają (minuty 1–480, ustawienia w widełkach, size→M, opacity→1, pos→null, idx→0, spent→0, poms→100). **v1.3.3: `clampInt` na `Number()`+`Number.isFinite` (A5 — odrzuca „ogon” typu „42abc”/„12px”, semantyka `null`/`''`→dflt zachowana, regresja 21/21 == stary parseInt); `dedupeIds` przy wczytaniu listy (A4 — zdublowane `id` z ręcznie zepsutego pliku dostają świeży `Date.now()+random`, format zadania bez zmian)** | 2026-07-15 (harness sanityzacji A4/A5) |
...
| Autoukrywanie paska zadań | ~~OTWARTE (O6)~~ **NAPRAWIONE A2 (v1.3.3):** `trayBounds()` clampuje dolny `y = min(workArea.y+height, bounds.y+bounds.height − taskbarH)` — dolna krawędź listwy nie schodzi poniżej ekranu. Tryb normalny bez zmian (min() nic nie rusza). Zmieniono TYLKO `y` (szerokość 28%/prawa krawędź nietknięte). Zweryfikowane harnessem geometrii (normalny: y=1032 bez zmian; autohide: bottom 1116→1080 = na ekranie; pasek górny też na ekranie). Fizyczny toggle autohide = opcjonalny test usera | 2026-07-15 (harness trayBounds) |
...
| Nieobsłużony wyjątek w callbacku interwału (A1 v1.3.3) | siatka `process.on('uncaughtException'/'unhandledRejection')` rejestrowana NAJWCZEŚNIEJ (po importach, przed oknami/interwałami) + `try/catch` w `tick()` i `reassertTopmost()`: pojedynczy rzut nie ubija procesu, interwał tyka dalej, `before-quit → flushSaved` (zapis postępu) chroniony. Tylko log, bez zmiany logiki | 2026-07-15 (harness A1: proces przeżywa 7 rzutów z interwału + node --check) |
...
- **`movingUntil` (main.js) — ZNANY, NIESZKODLIWY MARTWY KOD (A6, nie „naprawiać").** Zmienna
  `let movingUntil = 0` bramkuje `reassertTopmost`/watchdog/reclamp na czas przeciągania okna, ale
  drag widgetu obsługuje renderer (nie main), więc nigdy nie jest ustawiana ≠ 0 — warunki `Date.now() <
  movingUntil` są zawsze false. Zostaje jako rezerwa zgodności/dokumentacja intencji; usuwanie = zmiana
  bez wartości i ryzyko regresji przy ew. przyszłym dragu z main. Świadomie nietknięte w audycie v1.3.3.
...
- Manager: `body{overflow-y:auto}` BEZ `overflow-x:hidden` → dymki (data-tip) przy prawej krawędzi wystają
  ...
  **WDROŻONE A7 (v1.3.3):** reguła `[data-tip].tip-end:hover::after { left:auto; right:0; transform:none }`
  + klasa `tip-end` na **elementach REALNIE przy prawej krawędzi** (Jak to działa, Eksportuj CSV, „zacznij teraz”,
  Resetuj timer, Autostart, Folder logów, usuń zadanie). Klasa przeżywa konwersję `title→data-tip`
  (`convertTips` usuwa tylko `title`). overflow-x:hidden ZOSTAJE. NIE dotyka CSS listwy `body.role-tray`
  (M3 dla listwy bez zmian — inny plik, overlay.html).
  **KOREKTA po pomiarze CDP (2026-07-15):** „🎓 Samouczek" (`onboarding-btn`) NIE dostaje `tip-end` — nie jest
  przy prawej krawędzi (na prawo od niego jest „Jak to działa"), więc wyrównanie do prawej zakotwicza dymek
  do prawej krawędzi PRZYCISKU i długi tekst wychodzi w lewo (zmierzone: [−35, 339] px = ucięcie 35 px). Dla
  tego przycisku dymek zostaje WYŚRODKOWANY, a jego (najdłuższy) tekst skrócono „…uruchomienia (ekrany
  wprowadzające)" → „…pierwszego uruchomienia", żeby mieścił się wyśrodkowany (zmierzone: [168, 405] px w oknie
  528 px). **Ślepa uliczka:** NIE dawać `tip-end` elementom, które nie są przy samej prawej krawędzi kontenera —
  pogarsza (ucięcie z lewej) zamiast pomagać.
  Weryfikacja: **pomiar CDP na packaged (2026-07-15)** — 5 elementów prawej strefy z `tip-end` mieści się
  (np. Eksportuj CSV [205,504], „zacznij teraz” [218,493], Folder logów [273,504]); `onboarding-btn`
  wyśrodkowany+skrócony [168,405]. Wszystkie ≤ szerokości okna, brak poziomego scrolla (`scrollWidth==clientWidth`).

## 13. Build i wydanie (skrót — pełny proces: `C:\PROJEKTY\aplikacje\mojepomodoro\dev-docs\RELEASE_PLAYBOOK_MojePomodoro.md`)
